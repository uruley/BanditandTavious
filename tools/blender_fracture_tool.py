...
def run():
    ...
    # 4. Rename shards and fix origins
    shards = [o for o in bpy.context.selected_objects if o != obj]
    for i, s in enumerate(shards):
        bpy.context.view_layer.objects.active = s
        bpy.ops.object.origin_set(type='ORIGIN_CENTER_OF_MASS', center='BOUNDS')
        s.name = f"{{obj.name}}_shard_{{i}}-rigid"
...
def run():
    ...
    obj.name = "{name}"
    obj.scale = (0.5, 0.5, 2) # Tall pillar
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "setup":
        print(json.dumps(create_pillar(), indent=2))
    else:
